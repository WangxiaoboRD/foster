	  ...
	  $(function(){
		  ...
		  //生成ligerUI form
	  	 form = $("form").ligerForm({
             ...
             fields: [
                { display: "类型编码", name: "e.id", group: "死亡类型", groupicon: groupicon,type: "text", options:{readonly:true,value: '${e.id}'}}, 
                { display: "死亡类型", name: "e.name",validate: { required: true }, type: "text", options:{value: '${e.name}'}}  
               ]
	  		 });
	  		 v= form.validateForm();
	  	});
