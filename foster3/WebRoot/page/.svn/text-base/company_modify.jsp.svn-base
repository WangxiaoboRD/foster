		 ...
		 //生成ligerUI form
	  	 form = $("form").ligerForm({
             ...
             fields: [
				...
				{ display: "名称", name: "e.name", validate: { required: true }, newline: true, group: "基础信息", groupicon: groupicon,options: { value: '${e.name}',readonly:true },type: "text"},
                { display: "编码", name: "e.code", options: { value: '${e.code}' }, validate: { required: true },newline: true, type: "text" }, 
                { display: "地址", name: "e.address", newline: true,options: { value: '${e.address}' }, type: "text"}
                ]
	  		 });
	  		 v= form.validateForm();
	  	});
