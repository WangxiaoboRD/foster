 ...
 * 是否添加创建组件时的父元素引用：灵感来自Ext JS中  parentElement
 * 为了此处的应用我们添加了一个函数： setVisible >> UIComponent
 * 以下@add标记为扩充该ligerUI必须要明白的，在核心基础类base.js中
 * @add  在 window.liger.core.Component 的构造函数中添加了 userParam属性 
 * 		 this.userParam = options ;
 *       在window.liger.core.Component  的原型对象中添加了原型方法：addParams、setParam、getParam
 *          addParams: function(params) {
				for (var p in params) {
					this.userParam[p] = params[p];
				}
			},
			setParam: function(key, value) {
				this.userParam[key] = value;
			},
			getParam: function(key) {
				return this.userParam[key];
			}
		   *******************  添加目的  ********************************************
		   * 在使用Tab、Dialog打开页签和对话框传递参数时可以在打开的页面中直接通过 getParam(key)     *
		   * 获得传递的参数															   *
		   * **********************************************************************
   @add   在window.liger = $.ligerui 对象中的run函数中在使用如下语句： 
   *         return new $.ligerui[ext.controlNamespace][plugin]($.extend(
						{}, $[ext.defaultsNamespace][plugin] || {},
						$[ext.defaultsNamespace][plugin + 'String'] || {},
						$[ext.defaultsNamespace][plugin + 'Options']||{},//zhangyq add  2013-6-25 18:26:02
						args.length > 0 ? args[0] : {}));
		 其中 $[ext.defaultsNamespace][plugin + 'Options']||{} 为添加的语句
		 *******************  添加目的  ********************************************
	     * 对原始ligerUI组件中的参数进行扩充和替换需要                                      *
	     * 凡是在使用例如：XButton.js 中 为满足扩充需求添加了$.ligerDefaults.ButtonOptions={}*
	     * 你可以在其扩充															 *
	     * *********************************************************************** 
   @add  在window.liger.core.UIComponent的原型对象中添加 setVisible函数如下：
         setVisible: function(value){
			var g = this,p = this.options ;
			// 如果存在父元素
			if(value){
				if(p.parentElement){
					p.parentElement.removeClass("l-hidden");
				}else{
					$(g.element).parent().removeClass("l-hidden");
					$(g.element).removeClass("l-hidden");
				}
			}else{
				if(p.parentElement){
					p.parentElement.addClass("l-hidden");
				}else{
					$(g.element).parent().addClass("l-hidden");
					$(g.element).addClass("l-hidden");
				}
			}
		} 
		 *******************  添加目的  ********************************************
	     * 通过添加css属性隐藏该组件的显示情况。                                      *
	     * *********** 添加原因 ****************************************************
	     * 在使用XForm定义的组件ligerForm组件 自动创建form时需要动态显示和隐藏其中一个组件时使用该扩充**
	     * 方法
	     * ***********************************************************************
...
(function($) {
	...
	$.extend(liger.core.Component.prototype, {
		...
		destroy: function (){
            $.ligerui.remove(this);
        },// zhangyq add page param transmit
        addParams: function(params) {
			for (var p in params) {
				this.userParam[p] = params[p];
			}
	...
	liger.core.UIComponent.ligerExtend(liger.core.Component, {
		...
		_init : function() {
			...
			if (attributes && attributes instanceof Array) {
				for ( var i = 0; i < attributes.length; i++) {
					...
					this.options[name] = $(this.element).attr(name) || this.options[name]; //zhangyq update 2013年6月27日10:42:36
				}
			}
		...
		destroy : function() {
			...
			this.options = null;
			liger.remove(this);
		},//zhangyq add method 2013-7-2 11:45:43
		setVisible: function(value){
			var g = this,p = this.options ;
			// 如果存在父元素
			if(value){
	...
	liger.toJSON = typeof JSON === 'object' && JSON.stringify ? JSON.stringify
			: function(o) {
				var f = function(n) {
					return n < 10 ? '0' + n : n;
...
(function($) {
	...
	.ligerExtend(
		...
		_render : function() {
			...
			$(".l-accordion-header", g.accordion).hover(
				function() {
					$(this).addClass(
			...
			$("> .l-accordion-header", g.accordion).each(
				function() {
...
*  
* Author daomi 2013 [ gd_star@163.com ] 
* update1: if (g.trigger('BeforeSelect'[node]) == false) return; 此句话有大的问题，已经更正了。
* update2 :  if (p.initValue != null) 语句后要添加
*           //根据文本来初始化 
...
(function ($){
    ...
    $.ligerDefaults.ComboBox = {
        ...
        onStartResize: null,
        onEndResize: null,
        hideOnLoseFocus: true, // 鼠标离开时是否隐藏列表，默认为隐藏
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _setWidth: function (value)
        {
            ...
            if (value > 20)
            {
                ...
                //g.textwrapper.css({ width: value }); //mxq 2014-05-24 注释掉
                if (!p.selectBoxWidth) {
        ...
        setTree: function (tree)
        {
            ...
            else
            {
                ...
                {
                    //if (g.trigger('BeforeSelect'[node]) == false) return; update zhangyq
						...
						_render : function() {
							...
							if (p.label) {
								g.labelwrapper = g.textwrapper.wrap(
										...
										.parent();
								g.labelwrapper
										.prepend('<div class="l-text-label" style="float:left;display:inline;">' + p.label + ':&nbsp</div>');
								g.textwrapper.css('float', 'left');
								if (!p.labelWidth) {
 ...
 *  
 * Author daomi 2013 [ gd_star@163.com ] 
 * 待优化1：  p.initValue 配置项的名称也可以更改的。并且值可以通过  format配置项的类型更改。
 * 待优化2：  日期精确到秒吧？ 是否有必要还存在思考，不过这个可以轻松的实现。
 * UPDATE1: 更新的不仅仅是js代码，伴随还有css样式表代码
 */
(function($) {
	$.fn.ligerDateEditor = function() {
		return $.ligerui.run.call(this, "ligerDateEditor", arguments);
	...
	$.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input,{
						...
						_render : function() {
							...
							g.body.minuteselector = $(".l-box-dateeditor-minuteselector", g.body);
  							g.body.secondselector = $(".l-box-dateeditor-secondselector", g.body);//zhangyq add TODO
							
							g.toolbar.time = $(".l-box-dateeditor-time",g.toolbar);
							g.toolbar.time.hour = $("<a></a>");
							g.toolbar.time.minute = $("<a></a>");
							g.toolbar.time.second = $("<a></a>");//zhangyq add TODO TODO
							g.buttons = {
							...
							g.now = {
								...
								hour : nowDate.getHours(),
								minute : nowDate.getMinutes(),
								second: nowDate.getSeconds()// zhangyq add TODO
							};
							//当前的时间
							g.currentDate = {
								year : nowDate.getFullYear(),
							...
							if (p.showTime) {
								g.toolbar.time.show();
								g.toolbar.time.append(g.toolbar.time.hour).append(":").append(g.toolbar.time.minute).append(":").append(g.toolbar.time.second);//zhangyq add TODO
								$("li", g.body.hourselector).each(
										function(i, item) {
											var str = i;
											if (i < 10)
            ...
            $("li", g.body.secondselector).click(function (){
                ...
                g.bulidContent();
						...
						bulidContent : function() {
							...
							if (g.toolbar.time.second.html().length == 1)//zhangyq add TODO TODO
												...
												$("td", tr).each(function(j, td) {
																	...
																	if (g.selectedDate
																		...
																		$(td).addClass("l-box-dateeditor-out").removeClass("l-box-dateeditor-selected");
																			...
																			|| j == 6) {
																		$(td).addClass("l-box-dateeditor-holiday").removeClass(
																						"l-box-dateeditor-selected");
																	} else {
																		td.className = "";
																	}
						...
						showDate : function() {
							...
							this.currentDate.minute = parseInt(
									g.toolbar.time.minute.html(), 10);
							this.currentDate.second = parseInt(g.toolbar.time.second.html(), 10);//zhangyq add TODO
							var dateStr = this.currentDate.year + '/'
									+ this.currentDate.month + '/'
									+ this.currentDate.date + ' '
									+ this.currentDate.hour + ':'
						...
						isLongDateTime: function (dateStr){
				            var g = this, p = this.options;                               //zhangyq add TODO
				            var reg = /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/;
				            var r = dateStr.match(reg);
				            if (r == null) return false;		 //zhangyq add TODO
				            var d = new Date(r[1], r[3] - 1, r[4], r[5], r[6],r[7]);
				            if (d == "NaN") return false;														 //zhangyq add TODO	
						...
						onTextChange : function() {
							...
							g.selectedDate = {
								...
								hour : g.usedDate.getHours(),
								minute : g.usedDate.getMinutes(),
								second: g.usedDate.getSeconds()//zhangyq add TODO
							};
							g.currentDate = {
								year : g.usedDate.getFullYear(),
								month : g.usedDate.getMonth() + 1, //注意这里
...
(function($) {
	...
	$.ligerDefaults.Dialog = {
		...
		id : null, //给dialog附加id     liger.get("id").close();
		buttons : null, //按钮集合 
		isDrag : true, //是否拖动
		width : 280, //宽度
	...
	l.controls.Dialog.ligerExtend(l.core.Win,{
						...
						_render : function() {
							...
							if (p.target) {
								...
								if (p.load) {
									...
									// 加入以下代码 
									tmpId = 'jquery_ligerui_' + new Date()
											.getTime();
						...
						show : function() {
							...
							//前端显示 
							$.ligerui.win.setFront.ligerDefer($.ligerui.win,
									100, [ g ]);
						},
						setUrl : function(url) {
							this._setUrl(url);
										...
										.ligerDrag( {
											...
											onStopDrag : function() {
												...
												if (p.target) {
													...
													var triggers2 = l
															.find($.ligerui.controls.ComboBox);
													//更新所有下拉选择框的位置
													$(
															$.merge(triggers1,
																	triggers2))
...
(function($) {
	...
	$.ligerui.controls.DisplayField.ligerExtend($.ligerui.controls.Input,{
				...
				_render : function() {
					...
					g.inputText = $(this.element);
					//外层
					g.wrapper = g.inputText.wrap('<div class="l-text l-text-readonly"></div>').parent();
					g.wrapper.append('<div class="l-text-l"></div><div class="l-text-r"></div>');
					if (!g.inputText.hasClass("l-text-field"))
						g.inputText.addClass("l-text-field");
					g.set(p);
...
(function ($)
{
    ...
    //zhangyq add 
    $.ligerMethos.Drag = $.ligerMethos.Drag || {};
    // end of zhangyq add 
    ...
    l.controls.Drag.ligerExtend(l.core.UIComponent, {
        ...
        //zhangyq add  2013-7-27 16:00:18
        _extendMethods : function() {
			return $.ligerMethos.Drag;
		},
        //end of zhangyq add 
        _render: function ()
        {
...
(function($) {
	...
	$.ligerMethos.EasyTab = $.ligerMethos.EasyTab || {};
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        _initBuildHeader: function ()
        {
            ...
            if (p.title)
            {
                $("span", g.header).html(p.title);
                if (p.headerImg) {
                    $("span", g.header).append("<img src='" + p.headerImg + "' />").addClass("l-panel-header-hasicon");
                }
            }
            else
            {
...
(function($) {
	...
	$.ligerui.controls.ListBox = function(element, options) {
		$.ligerui.controls.ListBox.base.constructor
				.call(this, element, options);
	};
	$.ligerui.controls.ListBox
			.ligerExtend(
...
(function ($)
{
    ...
    $.ligerMethos.MessageBox =$.ligerMethos.MessageBox ||{};
...
(function ($)
{
    ...
    //zhangyq add 
    $.ligerMethos.Resizable = $.ligerMethos.Resizable || {};
    // end of zhangyq add 
...
(function($) {
	...
	$.ligerui.controls.Spinner.ligerExtend($.ligerui.controls.Input,{
			...
			_render : function() {
				...
				g.wrapper.append('<div class="l-text-l"></div><div class="l-text-r"></div>');
				g.wrapper.append(g.link) ;// delete  .after(g.selectBox).after(g.valueField); zhangyq delete
...
(function($) {
	...
	$.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input,{
				...
				_setValue : function(value) {
					// mxq 2013-11-11add start
					var g = this, p = this.options;
					if (p.render != null) {
						value = p.render(value);
					}
					// mxq 2013-11-11add end
...
(function($) {
	...
	$.ligerui.controls.Tip.ligerExtend(
					$.ligerui.core.UIComponent,
					{
						__getType : function() {
							return 'Tip';
...
(function($) {
	...
	// zhangyq add 2013-6-26 11:31:24
	$.ligerMethos.Tree = $.ligerMethos.Tree || {};
	// end of zhangyq add 2013-6-26 11:31:32
	...
	$.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent,{
		...
		//zhangyq add 2013-6-26 11:32:50
		_extendMethods : function() {
			return $.ligerMethos.Tree ;
		},
		// end of zhangyq 2013-6-26 11:33:08
		_render : function() {
			var g = this, p = this.options;
			g.set(p, true);
			g.tree = $(g.element);
		...
		remove : function(treeNode) {
			...
			var treenodedata = g._getDataNodeByTreeDataIndex(
					g.data, treedataindex);
			if (treenodedata)
				g._setTreeDataStatus( [ treenodedata ],
						'delete');
		...
		_getTreeHTMLByData : function(data, outlineLevel,
			...
			for ( var i = 0; i < data.length; i++) {
				...
				if (isExpandCurrent)
					treehtmlarr
							.push('isexpand=' + o.isexpand + ' ');
				treehtmlarr
						.push('outlinelevel=' + outlineLevel + ' ');
				//增加属性支持
				...
				if (g.hasChildren(o)) {
					...
					if (p.parentIcon) {
						...
						treehtmlarr
								.push(g
										._getParentNodeClassName(p.parentIcon ? true
												: false)
										+ " ");
						if (p.iconFieldName
								&& o[p.iconFieldName])
						...
						if (p.iconFieldName
								&& o[p.iconFieldName])
							treehtmlarr
									.push('<img src="' + o[p.iconFieldName] + '" />');
				...
				treehtmlarr
						.push('<span>' + o[p.textFieldName] + '</span></div>');
				if (g.hasChildren(o)) {
					var isLastNew = [];
...
(function($) {
	...
	$.ligerMethos.Window = $.ligerMethos.Window || {};
...
(function($) {
	...
	$.ligerMethos.Button = {
        ...
        // 设置可见，mxq 2015-02-07 add
        setVisible: function (){
			this.set('visible', true);
        },
        // 设置不可见，mxq 2015-02-07 add
 ...
 * 
 * 将ligerCheckBoxGroup中的split配置项禁用掉，不可进行配置，分隔符统一用逗号（，）号分隔
 */
(function($) {
...
(function($) {
	...
	$.ligerMethos.Check = {
		...
		getChecked : function() {
			var g = this, p = this.options;
			return g._getValue() == p.checkedValue;
		},
		// 外部方法：外部方法，设置选中状态
