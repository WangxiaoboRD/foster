...
import com.zd.epa.permission.entity.Role;
import com.zd.epa.permission.entity.RoleAuthValueDetail;
import com.zd.epa.permission.entity.UserAuthValue;
import com.zd.epa.permission.entity.Users;
import com.zd.epa.permission.services.IAuthFieldService;
import com.zd.epa.permission.services.IBussObjAuthPropertyService;
import com.zd.epa.permission.services.IRoleAuthValueDetailService;
import com.zd.epa.permission.services.IUserAuthValueService;
...
public class DataAuthUtil extends Base {
	...
	public <T> String buildDataAuthorityEntity(Class<T> clazz,Users u)throws Exception{
		...
		if( t != null && t.getId() != null && !"".equals(t.getId())){
			//判断如果查询对象包含Batch属性
			String className = clazz.getSimpleName();
			if(TypeUtil.isIncludeFieldBySelf("batch",clazz) || TypeUtil.isIncludeFieldBySelf("outBatch",clazz)){
				//获取所有技术员的batch
				List<Batch> blist =batchService.selectByHQL("from Batch e where e.technician.id='"+t.getId()+"' and e.isBalance='N'");
				if(blist != null && !blist.isEmpty()){
					...
					if(!"".equals(batchIds)){
						...
						if("FeedTransfer".equals(className) || "DrugTransfer".equals(className)){
							//去除最后一位“，”
							hql += " and e.outBatch.id in ("+batchIds+") and e.inBatch in ("+batchIds+")";
						}else
							hql += " and e.batch.id in ("+batchIds+")";
					}
				}else{
					   hql += " and e.batch.id is null";
				}
			}else if("Batch".equals(className)){
				List<Batch> blist =batchService.selectByHQL("from Batch e where e.technician.id='"+t.getId()+"' and e.isBalance='N'");
				if(blist != null && !blist.isEmpty()){
					String batchIds = "";
					for(Batch _b : blist)
					...
					if(!"".equals(batchIds)){
						batchIds = batchIds.substring(0,batchIds.length()-1);
						hql += " and e.id in ("+batchIds+")";
					}
			   }else{
				   hql += " and e.id is null";
			   }
				//hql += " and e.farmer.technician.id='"+t.getId()+"'";
				   
			}else if("Farmer".equals(className)){
				hql += " and e.technician.id= '"+t.getId()+"'";
			}
		}
		//--------------------------------------------------------------------
